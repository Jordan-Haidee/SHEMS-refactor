...
class HEMSEnv(gym.Env):
    def __init__(self, config_path: str = Path(__file__).parent / "default.toml") -> None:
        with open(config_path, encoding="utf-8") as f:
            self.config = Dict(pytoml.load(f))
        self.eta_ess = self.config.eta_ess
        self.ess_level_init = self.config.ess_level_init
        self.ess_level_max = self.config.ess_level_max
        self.ess_level_min = self.config.ess_level_min
        ...
        self.A_hvac = self.config.A_hvac
        ...
        self.price_data_table = pd.read_csv(Path(__file__).parent / self.solar_data_path)

        # 设置动作空间和观测空间
        self.action_space = gym.spaces.Box(
            low=np.array([-self.p_ess_max, 0]), high=np.array([self.p_ess_max, self.p_hvac_max])
        )
        self.observation_space = gym.spaces.Box(
            low=np.array([0, 0, self.ess_level_min, 0, 0, 0, 0]),
            high=np.array(
                [
    ...
    def normalize_state(self):
        for i in range(len(self.observation_space.shape[0])):
            self.state[i] = (self.state[i] - self.data_scope[i][0]) / (self.data_scope[i][1] - self.data_scope[0])
    ...
    def step(self, a: np.ndarray):
        ...
        # 计算ESS下一时隙的储能
        if p_ess >= 0:
            p_ess_charge = np.clip(p_ess, 0, min((self.ess_level_max - ess_level) / self.eta_ess, self.p_ess_max))
            p_ess_discharge = 0
            p_ess = p_ess_charge
        else:
            p_ess_charge = 0
            p_ess_discharge = np.clip(-p_ess, 0, min((ess_level - self.ess_level_min) * self.eta_ess, self.p_ess_max))
            p_ess = -p_ess_discharge
        ess_level_next = ess_level + self.eta_ess * p_ess_charge - 1 / self.eta_ess * p_ess_discharge
